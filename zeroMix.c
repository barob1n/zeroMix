...
void velMix(float ***velTr, float ***velOutTr,int ns, int numTr,int iNdx, float***zeroTr, int *xNdxMn, int *xNdxMx, int iOrigin, int xOrigin, int rate,int iline, int xline, int window, int fill);
...
/*Open fileS as readable/writable binary*/
		...
		if(sscanf(argv[7], "%d", &iMax) != 1){
			printf("\niMax entered not valid. Exiting...");
			return -1;
		}
		if(sscanf(argv[8], "%d", &xMax) != 1){
			printf("\nxMax entered not valid. Exiting...");
			return -1;
		}	
		if(sscanf(argv[9], "%d", &window) != 1){
			printf("\nxWindow entered not valid. Exiting...");
			return -1;
		}	
		if(sscanf(argv[10], "%d", &fill) != 1){
			printf("\fill entered not valid. Exiting...");
			return -1;
		}else if(!( fill==0 || fill ==1 || fill==2)){
			printf("\nfill entered not valid. Exiting...");	
			return -1;
		}
	...
	fprintf(stderr,"\n*******************************************************************************\n");
      ...
      fprintf(stderr,"zeroMix  <in.sgy><zero.sgy><out.sgy><Num><iOrigin><xOrigin><iMax><xMax>\n");
      fprintf(stderr,"          <window><fill> \n\n");
      ...
      fprintf(stderr,"window: Height of cube. 0 =  mixing takes place in horizontal plane only.\n\n");
      fprintf(stderr,"fill: 0 = keep zeros, 1 = fill with original value\n ");
      fprintf(stderr,"     2 = fill with average of two adjacent MIXED values\n");
    fprintf(stderr, "\n*******************************************************************************\n");
      return 0;
    } 
...
/* ******************************************************************* */
	...
	while ( thdr.iline - iOrigin < numTr  && statusR==0){
		...
		memcpy(velOutTr[thdr.iline - iOrigin][thdr.xline -xOrigin], tempTrV,(ns+1)*sizeof(float));
	...
	velMix(velTr, velOutTr, ns, numTr,ilineCnt,zeroTr,xNdxMn,xNdxMx,iOrigin,xOrigin,rate,thdr.iline,thdr.xline,window,fill);
		...
		* trGth[1] -> trGth[0], trGth[2] -> [1], and tempTr -> trGth[numTr-1]*/
		for (k =0 ;k<numTr-1;++k){
			for(j=0;j<xMax-xOrigin+1;++j){
				...
				memcpy(velOutTr[k][j],velOutTr[k+1][j],(ns+1)*sizeof(float));
	...
	for (xNdx=xStart+(halfTr);xNdx <= xEnd-(halfTr); ++xNdx){
		/*walking down the samples in the trace*/
			for (samp=0; samp<ns; ++samp){		
				...
				/*UP TRACE*/
				i=1;
				while (i<=window && zeroTr[halfTr][xNdx][samp+i] !=0 && samp-i>=0 && samp+i<=ns){
					sumVel+=velTr[halfTr][xNdx][samp+i];
					++numVel;
					++i;
				}
				/*DOWN TRACE*/
				...
				/*UP TRACE and UP iline*/
				i=1;
				while (i<=window && zeroTr[halfTr][xNdx+i][samp+i] !=0 && samp-i>=0 && samp+i<=ns){
					sumVel+=velTr[halfTr][xNdx+i][samp+i];
					++numVel;
					++i;
				}
				/*DOWN TRACE and UP iline*/
				i=1;
				while (i<=window && zeroTr[halfTr][xNdx+i][samp-i] !=0 && samp-i>=0 && samp+i<=ns){
					sumVel+=velTr[halfTr][xNdx+i][samp-i];
					++numVel;
				...
				/*UP TRACE and UP xline*/
				i=1;
				while (i<=window && zeroTr[halfTr+i][xNdx][samp+i] !=0 && samp-i>=0 && samp+i<=ns){
					sumVel+=velTr[halfTr+i][xNdx][samp+i];
					++numVel;
					++i;
				}
				/*DOWN TRACE and UP xline*/
				i=1;
				while (i<=window && zeroTr[halfTr+i][xNdx][samp-i] !=0 && samp-i>=0 && samp+i<=ns){
					sumVel+=velTr[halfTr+i][xNdx][samp-i];
					++numVel;
				...
				if(fill == 0){
					...
					sumVel=(velOutTr[halfTr-1][xNdx][samp]  )/1.0;
					++numVel;	
					
					if(velOutTr[halfTr-1][xNdx][samp]==0 ){
						printf("\n\n You have bad value in the out data!!\n\n");
						printf("\n\n %f %f\n\n ", velOutTr[halfTr-1][xNdx][samp], velOutTr[halfTr][xNdx-1][samp]);
						printf("\n\n %d %d\n\n ", iline,xline);
						getchar();
					}
				}
